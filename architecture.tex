...
>>> from sympy import *
\end{verbatim}
will import all public SymPy functions into the global Python namespace. All
the examples in this paper assume that this has been run.
...
\verb|a, b, c = symbols('x y z')|. All examples in this paper will assume that
the symbols \verb|x|, \verb|y|, and \verb|z| have been assigned to the a symbol
...
exponentiation is \verb|**|. For instance, the following creates the expresion
$(x^2 - 2x + 3)/y$.
\begin{verbatim}
>>> (x**2 - 2*x + 3)/y
(x**2 - 2*x + 3)/y
...
dictionaries, therefore enabling caching and other features.
...
with them. In SymPy, every symbolic expression is an instance of properly
designed Python classes, representing expression trees. The operators are
represented by the type of an expression and the child nodes are stored in the
